...
struct D3D12Renderer::Implementation final {
    ...
    [[nodiscard]] bool createPipelines(ID3D12PipelineLibrary* pipelineLibrary) noexcept;
...
bool D3D12Renderer::Implementation::initialize(
    ...
    if (requested.pipelineLibrary != nullptr
        && !validateDeviceChild(
            *requested.pipelineLibrary,
            "D3D12 pipeline library belongs to a different device")) {
        shutdown();
        return false;
    }
    statistics = {};
...
bool D3D12Renderer::Implementation::createPipelines(
    ID3D12PipelineLibrary* pipelineLibrary) noexcept {
#if defined(HENIAUI_D3D12_RUNTIME_SHADER_COMPILATION)
    ...
    const D3D12_SHADER_BYTECODE vertexBytecode{
        vertexShader->GetBufferPointer(), vertexShader->GetBufferSize()};
    const D3D12_SHADER_BYTECODE pixelBytecode{
        pixelShader->GetBufferPointer(), pixelShader->GetBufferSize()};
    const D3D12_SHADER_BYTECODE textureFreePixelBytecode{
        textureFreePixelShader->GetBufferPointer(), textureFreePixelShader->GetBufferSize()};
#else
    const D3D12_SHADER_BYTECODE vertexBytecode{
        henia::backend::d3d12::generated::ui::kVertexShader,
        sizeof(henia::backend::d3d12::generated::ui::kVertexShader)};
    const D3D12_SHADER_BYTECODE pixelBytecode{
        henia::backend::d3d12::generated::ui::kPixelShader,
        sizeof(henia::backend::d3d12::generated::ui::kPixelShader)};
    const D3D12_SHADER_BYTECODE textureFreePixelBytecode{
        henia::backend::d3d12::generated::ui::kPixelVariantShader,
        sizeof(henia::backend::d3d12::generated::ui::kPixelVariantShader)};
#endif
    ...
    description.PS = textureFreePixelBytecode;
    description.BlendState = blendDescription(false);
    if (!createPipeline(
            pipelineLibrary,
...
bool D3D12Renderer::Implementation::createPipeline(
    ID3D12PipelineLibrary* pipelineLibrary,
    const PipelineName& name,
    const D3D12_GRAPHICS_PIPELINE_STATE_DESC& description,
    ComPtr<ID3D12PipelineState>& output,
